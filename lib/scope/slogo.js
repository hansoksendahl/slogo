...
//        `~~~~~~~~~~' ~~ ~~ ~~
//
// Slogo is a minimalist framework for wegGL.  It acts as a javascript interface
// for common WebGL tasks.
//
// By providing a wrapper for _nearly_ every function in the WebGL
// specification we gain a Javascript prototype we can inherit from.
//
// With Slogo calls are said to be _chainable_ since every function returns the
// Slogo instance.  This slight increase in overhead is worth it for the
// syntactic sugar alone.

// ## The Slogo class
//
// _Note: it's wrapped in a
// [closure](http://en.wikipedia.org/wiki/Closure_(computer_science)) like a
// protective layer of mucus_.
var Slogo = (function() {
  ...
  slogo.prototype = {
    // ### resizeTo
    //
    // Resizes the canvas and WebGL viewport.  Perfect for binding to
    // `window.onResize`.
    //
    // By calling `SlogoInstance.resizeTo('css')` we can resize the canvas to
    // whatever size its current CSS rules define.
    //
    //     resizeTo(x, y)
    //     resizeTo('css')
